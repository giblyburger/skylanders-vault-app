...
function renderPlacedTrap(placed, element) {
  ...
  return '<span class="placed-trap" style="' + style + '" aria-hidden="true">' +
    '<span class="placed-trap__cap"></span>' +
    '<span class="placed-trap__core"><span>' + escapeHtml(trap.mold) + '</span></span>' +
    '<span class="placed-trap__base"></span>' +
  '</span>';
}

export function renderVillainBoard(root, context) {
  const {
    elements,
    villains,
    state,
  ...
  root.innerHTML = groups.map((group) => {
    ...
    const slots = visibleVillains.map((villain) => {
      ...
      const justPlaced = Boolean(placed && lastPlacement?.villainId === villain.id && lastPlacement?.trapId === placed.trap.id);
      const actionLabel = hasSelectedTrap
        ? 'Place ' + selectedTrap.official + ' on ' + villain.name
        : 'Cycle ' + villain.name + ' status';
      ...
      return '<button class="villain-slot" type="button" data-villain-id="' + escapeHtml(villain.id) + '" data-status="' + status + '" data-placeable="' + (canPlace ? 'true' : 'false') + '" data-placement-mode="' + (hasSelectedTrap ? 'true' : 'false') + '" data-has-trap="' + (placed ? 'true' : 'false') + '" data-just-placed="' + (justPlaced ? 'true' : 'false') + '" style="' + style + '" aria-label="' + escapeHtml(actionLabel + '. ' + villain.name + ', ' + group.elementName + ', ' + label + placedLabel) + '">' +
        ...
        '<span class="hex-port" aria-hidden="true"><span></span></span>' +
        renderPlacedTrap(placed, group.element) +
        (justPlaced ? '<span class="capture-burst" aria-hidden="true"><i></i><i></i><i></i></span>' : '') +
        '<span class="slot-name">' + escapeHtml(villain.name) + '</span>' +
        '<span class="slot-meta">' + (villain.doomRaider ? 'Doom Raider · ' : '') + escapeHtml(label) + (placed ? ' · ' + escapeHtml(placed.trap.mold) : '') + '</span>' +
      '</button>';
    }).join('');

    return '<section class="element-bank" style="' + style + '" aria-label="' + escapeHtml(group.elementName) + ' villains">' +
      '<header class="element-bank__header">' +
        '<span class="element-token">' + elementIcon(group.element.icon) + '</span>' +
        '<div><h3>' + escapeHtml(group.elementName) + '</h3><p>' + trapped + '/' + group.villains.length + '</p></div>' +
      '</header>' +
