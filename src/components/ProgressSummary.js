...
function calculateProgress({ villains, traps, state }) {
...
function renderSummary(root, progress) {
  const items = [
  ...
  root.innerHTML = items.map(([label, value, percent]) => {
    return '<article class="stat" aria-label="' + label + ": " + value + '"><strong>' + value + "</strong><span>" + label + '</span><i style="--value:' + percent + '%"></i></article>';
  }).join("");
