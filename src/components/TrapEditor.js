...
function createTrapEditor(dialog, callbacks) {
  let activeTrap = null;
  let activeContext = null;
  dialog.innerHTML = '<form class="trap-editor" method="dialog" aria-labelledby="editor-title"><header class="trap-editor__header"><div><span class="editor-element" data-editor-element></span><h2 id="editor-title" data-editor-title></h2><p data-editor-subtitle></p></div><button class="icon-button close-button" type="button" data-editor-close aria-label="Close editor">' + actionIcon("close") + '</button></header><div class="editor-preview" data-editor-preview></div><label class="field">Owned quantity<input data-editor-qty type="number" min="0" max="99" step="1" inputmode="numeric"></label><label class="field">Assigned villain<select data-editor-assigned></select></label><footer class="trap-editor__actions"><button class="button button--danger" type="button" data-editor-missing>Set Missing</button><button class="button button--primary" type="submit">Save Trap</button></footer></form>';
  ...
  function save(quantityOverride) {
    ...
    const quantity = Math.max(0, Number(quantityOverride != null ? quantityOverride : qtyInput.value) || 0);
    callbacks.onSave(activeTrap.id, {
      quantity,
      assignedVillainId: quantity > 0 ? (selected == null ? void 0 : selected.dataset.villainId) || "" : "",
      assignedVillainName: quantity > 0 ? (selected == null ? void 0 : selected.dataset.villainName) || "" : ""
    });
    close();
  }
  ...
  function open(trap, context) {
    ...
    dialog.querySelector("[data-editor-subtitle]").textContent = trap.edition + " \xB7 " + trap.collectionGroup;
