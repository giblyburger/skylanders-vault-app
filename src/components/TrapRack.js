...
export function renderTrapRack(root, context) {
  const {
    elements,
    villainsById,
    traps,
  ...
  root.innerHTML = visibleTraps.map((trap) => {
    ...
    const details = [trap.edition, assignedName ? 'On board: ' + assignedName : 'Unplaced'].filter(Boolean).join(' · ');
    const preloaded = trap.preloaded ? '<span class="preloaded-chip">Factory loaded: ' + escapeHtml(trap.preloaded) + '</span>' : '';

    return '<article class="trap-card" data-trap-id="' + escapeHtml(trap.id) + '" data-owned="' + (owned ? 'true' : 'false') + '" data-selected="' + (selected ? 'true' : 'false') + '" style="' + style + '">' +
      '<button class="trap-card__select" type="button" data-trap-select-id="' + escapeHtml(trap.id) + '" aria-label="Pick up ' + escapeHtml(trap.official) + ' for board placement">' +
        '<span class="owned-pill">' + (owned ? 'Owned x' + record.quantity : 'Missing') + '</span>' +
        '<span class="selected-pill">Ready</span>' +
        '<div class="trap-crystal" aria-hidden="true">' +
          '<span class="crystal-cap"></span><span class="crystal-core"><span>' + escapeHtml(trap.mold) + '</span></span><span class="crystal-base"></span>' +
        '</div>' +
        '<div class="trap-card__body">' +
          '<span class="trap-card__eyebrow">' + elementIcon(element.icon) + escapeHtml(trap.element + ' · ' + trap.collectionGroup) + '</span>' +
          '<strong>' + escapeHtml(trap.official) + '</strong>' +
          '<span>' + escapeHtml(trap.element + ' ' + trap.mold) + '</span>' +
          '<small>' + escapeHtml(details) + '</small>' +
          preloaded +
        '</div>' +
      '</button>' +
      '<button class="trap-card__edit" type="button" data-trap-edit-id="' + escapeHtml(trap.id) + '" aria-label="Edit ' + escapeHtml(trap.official) + ' details" title="Edit Trap">' + actionIcon('edit') + '<span>Details</span></button>' +
    '</article>';
