...
function copyMarkup(copy, index) {
  ...
  return `<article class="catalog-copy" data-copy="${escapeHtml(copy.id)}">
    ...
    <section class="copy-photos" aria-label="Personal photos for copy ${index + 1}">
      ...
      ${photos.length ? `<div class="copy-photo-grid">${photos.map((photo) => `<figure><button type="button" data-photo-viewer-src="${escapeHtml(photo.url)}" data-photo-viewer-alt="Personal photo of copy ${index + 1}" data-photo-viewer-label="${escapeHtml(photo.filename || `Copy ${index + 1} photo`)}"><img loading="lazy" src="${escapeHtml(photo.url)}" alt="Personal collection photo"></button><button class="copy-photo-delete" type="button" data-delete-photo="${escapeHtml(photo.id)}" data-copy-id="${escapeHtml(copy.id)}" aria-label="Delete this photo">×</button></figure>`).join('')}</div>` : '<p class="copy-photos__empty">On your phone, tap Add photo to open the camera. It will appear on your other signed-in devices after syncing.</p>'}
    </section>
  </article>`;
}
