        ...
        <div class="tools-two-column">
          <section class="feature-card">
            ...
            <div class="master-feature-list">
              ...
              ${featureRow('Gated NFC editing', 'Write or rewrite compatible blank or personally owned NDEF tags on supported Android devices. Original encrypted figure tags are never altered.')}
            </div>
          </section>
          <section class="feature-card">
  ...
  function identifyMarkup() {
    return `
      <section class="photo-lab">
        <header class="tool-section-head"><div><p class="eyebrow">Private visual matching</p><h3>Photo identification</h3><p>The selected photo stays in this browser. It is compared locally with the 640-item obtainable reference library and is never uploaded.</p></div></header>
            ...
            <div>
              ${ui.photoMatches.length ? ui.photoMatches.map((match, index) => `
                <button type="button" data-photo-match="${match.card.id}" style="--card-el:${elementColor(match.card.element)}">
                  <img src="assets/card-art/thumbs/${encodeURIComponent(match.card.id)}.webp" alt="">
