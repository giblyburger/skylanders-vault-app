...
const ACTIVE_POLL_MS = LEGACY_IPAD ? 6e3 : 1800;
const BACKGROUND_POLL_MS = LEGACY_IPAD ? 3e4 : 1e4;
function createCloudSync({ getState, normalizeState, applyState, onStatus, onPairingRequired }) {
  let revision = 0;
  ...
  async function request(path, options = {}) {
    ...
    if (!response.ok) {
      ...
      if (response.status === 401 && path !== "/api/pair") {
        locked = true;
        onPairingRequired == null ? void 0 : onPairingRequired();
      }
      throw error;
    }
    return payload;
  ...
  async function start() {
    ...
    try {
      ...
      if (remote.state) {
        ...
        const merged = localHasData && !same(local, normalizedRemote) ? mergeStates(baseState, local, normalizedRemote) : normalizedRemote;
  ...
  async function save() {
    ...
    try {
      ...
      if (error.status === 409) {
        const remote = normalizeState(((_a = error.payload) == null ? void 0 : _a.state) || {});
        const merged = mergeStates(baseState, local, remote);
        revision = Number(((_b = error.payload) == null ? void 0 : _b.revision) || 0);
  ...
  async function uploadPhoto(cardId, copyId, file) {
    ...
    const result = await request("/api/photos?".concat(params), {
      method: "POST",
      headers: {
  ...
  async function deletePhoto(photoId) {
    await request("/api/photos/".concat(encodeURIComponent(photoId)), { method: "DELETE" });
  }
...
function mergeStates(base, local, remote) {
  const merged = { version: Math.max(Number((local == null ? void 0 : local.version) || 0), Number((remote == null ? void 0 : remote.version) || 0), 6) };
  ["villains", "traps", "catalog"].forEach((section) => {
    const baseMap = (base == null ? void 0 : base[section]) || {};
    const localMap = (local == null ? void 0 : local[section]) || {};
    const remoteMap = (remote == null ? void 0 : remote[section]) || {};
    merged[section] = {};
    const keys = /* @__PURE__ */ new Set([...Object.keys(baseMap), ...Object.keys(localMap), ...Object.keys(remoteMap)]);
    keys.forEach((key) => {
      const localChanged = !same(localMap[key], baseMap[key]);
      const value = localChanged ? localMap[key] : remoteMap[key];
      if (value !== void 0) merged[section][key] = clone(value);
    });
  });
  return merged;
}
function hasCollectionData(state) {
  return ["villains", "traps", "catalog"].some((section) => Object.keys((state == null ? void 0 : state[section]) || {}).length > 0);
}
...
function getDeviceId() {
  ...
  if (!id) {
    id = ((_a = crypto.randomUUID) == null ? void 0 : _a.call(crypto)) || "device-".concat(Date.now(), "-").concat(Math.random().toString(36).slice(2));
